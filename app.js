...
function validateIncidents(data) {
  ...
  data.forEach((inc, index) => {
    ...
    const nameStr = String(inc.name || inc.title || titleStr).toUpperCase();
    ...
    const normalized = {
      ...
      name: nameStr,
      title: titleStr,
      date: String(inc.date || "UNKNOWN DATE"),
      year: parsedYear,
      location: String(inc.location || "UNKNOWN LOCATION"),
      country: String(inc.country || ""),
      coords: [lat, lng],
      status: rawStatus,
      type: String(inc.type || inc.category || "UNIDENTIFIED AERIAL PHENOMENON").toUpperCase(),
      description: String(inc.description || "No description recorded."),
...
// Use the ocean's projected outline so the sweep follows the map when zooming.
const radarSphere = { type: 'Sphere' };
svg.append('defs').append('clipPath')
  .attr('id', 'map-radar-clip')
  .attr('clipPathUnits', 'userSpaceOnUse')
  .append('path').attr('d', path(radarSphere));

function appendMapRadarSweep() {
  const [[left, top], [right, bottom]] = path.bounds(radarSphere);
  const sweep = g.append('g')
    .attr('clip-path', 'url(#map-radar-clip)')
    .attr('pointer-events', 'none')
    .attr('aria-hidden', 'true')
    .append('foreignObject')
    .attr('x', left).attr('y', top)
    .attr('width', right - left).attr('height', bottom - top)
    .append('xhtml:div').attr('class', 'map-radar-sweep');

  // Read the sweep's animation clock so marker flashes stay aligned with it.
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)');
  let previousTime = null;
  function syncRadarHits() {
    const animation = sweep.node().getAnimations({ subtree: true })[0];
    const time = animation && Number(animation.currentTime);
    if (!reducedMotion.matches && !document.hidden && animation && Number.isFinite(time)) {
      const duration = Number(animation.effect.getTiming().duration);
      // Match the CSS sweep's seven-second travel within its twelve-second cycle.
      const travelDuration = duration * (7 / 12);
      if (previousTime !== null && time >= previousTime && time - previousTime < 250) {
        g.selectAll('.inc-marker').each(function () {
          const fraction = (Number(this.dataset.radarY) - top) / (bottom - top);
          const hitTime = Math.floor(time / duration) * duration + fraction * travelDuration;
          if (hitTime > previousTime && hitTime <= time) {
            this.querySelectorAll('.marker-inner, .marker-outer').forEach(dot => {
              dot.animate([
                { opacity: 1, filter: 'brightness(1)' },
                { opacity: 1, filter: 'brightness(1.8)', offset: 0.15 },
                { opacity: 0.4, filter: 'brightness(1)', offset: 0.35 },
                { opacity: 1, filter: 'brightness(1.5)', offset: 0.55 },
                { opacity: 1, filter: 'brightness(1)' }
    ...
    requestAnimationFrame(syncRadarHits);
  }
  requestAnimationFrame(syncRadarHits);
}
...
function loadMapData() {
  d3.json('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json')
    .then(world => {
      ...
      // Geographic labels share the map projection and pan/zoom transform.
      const geographicLabels = [
        { name: 'NORTH AMERICA', coords: [-105, 48], ocean: false },
        { name: 'SOUTH AMERICA', coords: [-60, -20], ocean: false },
        { name: 'EUROPE', coords: [20, 53], ocean: false },
        { name: 'AFRICA', coords: [20, 5], ocean: false },
        { name: 'ASIA', coords: [95, 45], ocean: false },
        { name: 'AUSTRALIA', coords: [134, -25], ocean: false },
        { name: 'ANTARCTICA', coords: [0, -80], ocean: false },
        { name: 'PACIFIC OCEAN', coords: [-140, -10], ocean: true },
        { name: 'ATLANTIC OCEAN', coords: [-35, 15], ocean: true },
        { name: 'INDIAN OCEAN', coords: [78, -25], ocean: true },
        { name: 'ARCTIC OCEAN', coords: [0, 79], ocean: true },
        { name: 'SOUTHERN OCEAN', coords: [20, -60], ocean: true }
      ];
      g.append('g')
        .attr('class', 'geographic-labels')
        .attr('pointer-events', 'none')
        .selectAll('text')
        ...
        .attr('font-size', d => d.ocean ? 10 : 11)
        .attr('font-weight', d => d.ocean ? 500 : 600)
        .attr('font-style', d => d.ocean ? 'italic' : 'normal')
        .attr('letter-spacing', '1.2px')
        .attr('fill', d => d.ocean ? '#527c91' : '#8ba6b2')
        .attr('stroke', '#040d18')
        .attr('stroke-width', 2)
...
function calculateTooltipPosition(screenX, screenY, containerWidth, containerHeight, cardWidth, cardHeight) {
  ...
  left = Math.max(PADDING, Math.min(left, Math.max(PADDING, containerWidth - cardWidth - PADDING)));
...
function positionTooltip(inc, isPinned = false) {
  ...
  const transform = d3.zoomTransform(svg.node());
  let screenX = pos[0] * transform.k + transform.x;
  let screenY = pos[1] * transform.k + transform.y;

  const containerEl = document.getElementById('map-area') || container;
  const rect = containerEl.getBoundingClientRect();
  // SVG viewBox units differ from CSS pixels on a resized/responsive map.
  const screenMatrix = svg.node().getScreenCTM();
  if (screenMatrix) {
    const point = svg.node().createSVGPoint();
    point.x = screenX;
    point.y = screenY;
    const screenPoint = point.matrixTransform(screenMatrix);
    screenX = screenPoint.x - rect.left - containerEl.clientLeft;
    screenY = screenPoint.y - rect.top - containerEl.clientTop;
  }
